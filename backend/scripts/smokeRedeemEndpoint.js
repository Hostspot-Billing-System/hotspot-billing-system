...
async function main() {
  ...
  try {
    ...
    const code = `SMOKE-REDEEM-ENDPOINT-${Date.now()}`;
    ...
    const redeemRes = await requestJson(`${base}/api/vouchers/redeem`, {
      method: 'POST',
    ...
    console.log(JSON.stringify(redeemRes, null, 2));
  } finally {
...
main().catch((err) => {
  // eslint-disable-next-line no-console
  console.error('Smoke redeem endpoint failed:', err?.message ?? err);
  process.exit(1);
});
