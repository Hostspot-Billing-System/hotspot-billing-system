...
export class WithdrawalsService {
  static async getWithdrawableBalance({ agent_id, client_id } = {}) {
    const summary = await WithdrawalsService.getWithdrawableSummary({ agent_id, client_id });
    return summary.withdrawable_amount_ugx;
  }

  static async getWithdrawableSummary({ agent_id, client_id } = {}) {
    ...
    const result = await query(
      `
      SELECT
        COALESCE(SUM(t.amount), 0)::float8 AS total_earnings,
        COALESCE(SUM(t.commission_amount), 0)::float8 AS commission_deducted,
        COALESCE(SUM(t.net_amount), 0)::float8 AS withdrawable
      FROM transactions t
      ${whereSql}
      `,
      params
    ...
    return {
      total_earnings_ugx: Number(row.total_earnings ?? 0),
      commission_deducted_ugx: Number(row.commission_deducted ?? 0),
      withdrawable_amount_ugx: Number(row.withdrawable ?? 0),
    };
  }
